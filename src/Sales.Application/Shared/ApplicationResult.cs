...
public class ApplicationResult : ValueResult
{
    ...
    private ApplicationResult(bool succeeded, IReadOnlyList<FailureDetail>? failureDetails, HttpStatusCode status) : base(succeeded, failureDetails)
    {
        Status = status;
    }
    ...
    public static ApplicationResult BadRequest(IReadOnlyList<FailureDetail> failuresDetail) => new(false, failuresDetail, HttpStatusCode.BadRequest);
...
public sealed class ApplicationResult<TValue> : ValueResult<TValue> where TValue : class
{
    ...
    private ApplicationResult(bool succeeded, TValue? value, IReadOnlyList<FailureDetail>? failureDetails, HttpStatusCode status) : base(succeeded, value, failureDetails)
    {
        Status = status;
    }
    ...
    public static ApplicationResult<TValue> BadRequest(IReadOnlyList<FailureDetail> failuresDetail) => new(false, null, failuresDetail, HttpStatusCode.BadRequest);
